...
- (void) configureConnectionToProduct {
    ...
    int counter = 0;
    // Infinite while loop to retry connection
    // if connection succeeds, we exit the loop
    ...
    while (self->_retryConnection) {
        if ([DJISDKManager startConnectionToProduct]){
            ...
            _uavConnectionStatusLabel.text = [NSString stringWithFormat:@"UAV Status: Connection Reattempt %d",counter];
        }
    }
